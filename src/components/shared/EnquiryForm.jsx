...
const initialState = { name: "", email: "", phone: "", quantity: "", message: "" };
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          ...
          company_website: e.target.company_website?.value || "",
        }),
      });
  ...
  if (status === "success" && compact) {
    return (
      <div className="rounded-[3px] border border-border bg-steel-soft/60 p-5 text-sm text-ink">
        Thanks — your enquiry about <strong>{productName}</strong> has been sent.
